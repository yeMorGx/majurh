...
type Member = { user_id: string; email: string | null; full_name: string | null; is_active: boolean; must_change_password: boolean; onboarding_completed_at: string | null; preferred_name?: string | null; birth_date?: string | null; phone?: string | null; lead_source?: string | null; referral_name?: string | null; primary_goal?: string | null; organization_name?: string | null; created_at: string };
...
export function AdminConsole() {
  ...
  async function createUser(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await request('/api/admin/users', { method: 'POST', body: JSON.stringify({ email: userForm.email, password }) });
      setCreatedCredentials({ email: userForm.email, password });
  ...
  async function removeMember(member: Member) {
    if (!window.confirm(`Revogar o acesso geral de ${memberDisplayName(member)}? A conta será preservada, mas não poderá iniciar uma sessão ativa no Majurh.`)) return;
    setError('');
    try {
...
function MemberRow({ member, compact = false, onEdit, onRemove, onReactivate }: { member: Member; compact?: boolean; onEdit?: () => void; onRemove?: () => void; onReactivate?: () => void }) {
  const onboardingLabel = member.onboarding_completed_at ? 'Onboarding concluído' : 'Primeiro acesso';
  const name = memberDisplayName(member);
  return <div className={`admin-member-row ${compact ? 'is-compact' : ''}`}><div className="admin-member-initials" aria-hidden="true">{initials(name)}</div><div className="admin-member-identity"><strong>{name}</strong><span>{member.email || 'E-mail não informado'}</span></div><span className={`admin-role-badge ${member.is_active ? 'role-active' : 'role-revoked'}`}>{member.is_active ? onboardingLabel : 'Acesso revogado'}</span>{!compact && <div className="admin-member-actions"><button className="admin-row-button" onClick={onEdit}>Editar senha</button>{member.is_active ? <button className="admin-row-button is-danger" onClick={onRemove}>Revogar</button> : <button className="admin-row-button" onClick={onReactivate}>Reativar</button>}</div>}</div>;
}
...
function EditMemberDialog({ member, onClose, onSubmit }: { member: Member; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return <div className="admin-dialog-backdrop" role="presentation"><dialog open className="admin-dialog" aria-labelledby="edit-member-title"><div className="admin-dialog-header"><div><span className="admin-panel-label">Administrar acesso geral</span><h2 id="edit-member-title">{memberDisplayName(member)}</h2></div><button className="admin-close-button" onClick={onClose} aria-label="Fechar">×</button></div><p>{member.email || 'E-mail não informado'}</p><div className="admin-profile-summary"><span className="admin-panel-label">Dados do onboarding</span><div><span>Nome preferido</span><strong>{member.preferred_name || 'Ainda não preenchido'}</strong></div><div><span>Nascimento</span><strong>{member.birth_date ? formatDate(member.birth_date) : 'Ainda não preenchido'}</strong></div><div><span>Telefone / WhatsApp</span><strong>{member.phone || 'Ainda não preenchido'}</strong></div><div><span>Como conheceu</span><strong>{leadSourceLabel(member.lead_source, member.referral_name)}</strong></div><div><span>Objetivo principal</span><strong>{goalLabel(member.primary_goal)}</strong></div><div><span>Organização</span><strong>{member.organization_name || 'Ainda não criada'}</strong></div></div><form className="admin-form" onSubmit={onSubmit}><Field label="Nova senha" htmlFor="edit-password"><input id="edit-password" name="password" minLength={8} placeholder="Informe uma nova senha" required /></Field><small>Esta senha é exclusiva do acesso ao Majurh. A organização será criada pela própria pessoa.</small><div className="admin-dialog-actions"><button className="admin-secondary-button" type="button" onClick={onClose}>Cancelar</button><button className="admin-primary-button" type="submit">Salvar senha</button></div></form></dialog></div>;
}

function CredentialsDialog({ credentials, onClose }: { credentials: { email: string; password: string }; onClose: () => void }) {
...
function memberDisplayName(member: Pick<Member, 'full_name' | 'email'>) {
  return member.full_name?.trim() || member.email?.split('@')[0] || 'Acesso sem nome';
}
function initials(name: string | null | undefined) { return name?.trim().split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]).join('').toUpperCase() || '??'; }
...
function leadSourceLabel(value?: string | null, referralName?: string | null) { const labels: Record<string, string> = { linkedin: 'LinkedIn', referral: `Indicação${referralName ? ` · ${referralName}` : ''}`, google: 'Busca no Google', instagram: 'Instagram / anúncio', event: 'Evento ou feira de RH', other: 'Outro' }; return value ? labels[value] || value : 'Ainda não preenchido'; }
function goalLabel(value?: string | null) { return ({ organize_hr: 'Organizar o RH', documents_payroll: 'Documentos e holerites', explore: 'Testar e explorar' } as Record<string, string>)[value || ''] || 'Ainda não preenchido'; }

...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ userId: string }> }) {
  try {
    ...
    const members = await context.db`select user_id, role from public.organization_members where organization_id = ${context.organizationId}::uuid and user_id = ${userId} limit 1` as Array<{ user_id: string; role: AppRole }>;
    ...
    if (nextRole && member.role === 'admin' && nextRole !== 'admin') {
      const admins = await context.db`select count(*)::int as count from public.organization_members where organization_id = ${context.organizationId}::uuid and role = 'admin'::public.app_role` as Array<{ count: number }>;
    ...
    if (nextRole && nextRole !== member.role) await context.db`update public.organization_members set role = ${nextRole}::public.app_role where organization_id = ${context.organizationId}::uuid and user_id = ${userId}`;
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ userId: string }> }) {
  try {
    ...
    const members = await context.db`select user_id from public.organization_members where organization_id = ${context.organizationId}::uuid and user_id = ${userId} limit 1` as Array<{ user_id: string }>;
    if (!members.length) return errorJson('Esta pessoa não pertence à organização.', 404);
    const removed = await context.db`delete from public.organization_members where organization_id = ${context.organizationId}::uuid and user_id = ${userId} returning user_id` as Array<{ user_id: string }>;

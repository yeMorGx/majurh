...
export function ProductivityClient() {
  ...
  return (
    <div className="productivity-page">
      ...
      {timerRunning && <ActiveTrackerOverlay seconds={timerSeconds} task={timerTask} onToggle={() => setTimerRunning((current) => !current)} onReset={() => { setTimerRunning(false); setTimerSeconds(0); }} />}
    </div>
  );
}
...
function ActiveTrackerOverlay({ seconds, task, onToggle, onReset }: { seconds: number; task: string; onToggle: () => void; onReset: () => void }) {

...
type Vacancy = { id: string; title: string; department: string | null; unit: string | null; quantity: number; company_id: string | null };
...
export function NewProcessForm({ organizationId, candidateId, onCreated, onCancel }: { organizationId: string; candidateId: string; onCreated: (process: Process) => void; onCancel: () => void }) {
  ...
  return <form className="inline-form-card" onSubmit={submit}><div className="inline-form-heading"><div><strong>Novo processo seletivo</strong><span>Crie uma nova participação para este candidato.</span></div><button type="button" className="icon-button" onClick={onCancel} aria-label="Fechar"><Icon name="x" size={16} /></button></div><div className="form-grid"><div className="field field-full"><label htmlFor="new-process-vacancy">Vaga</label><select className="form-select" id="new-process-vacancy" value={vacancyId} onChange={(event) => setVacancyId(event.target.value)}><option value="">Processo sem vaga definida</option>{vacancies.map((vacancy) => <option value={vacancy.id} key={vacancy.id}>{vacancy.title}{companyNames.get(vacancy.company_id ?? '') ? ` · ${companyNames.get(vacancy.company_id ?? '')}` : ''}{vacancy.unit ? ` · ${vacancy.unit}` : ''} · {vacancy.quantity} {vacancy.quantity === 1 ? 'posição' : 'posições'}</option>)}</select></div><div className="field"><label htmlFor="new-process-status">Status inicial</label><select className="form-select" id="new-process-status" value={status} onChange={(event) => setStatus(event.target.value)}>{processStatuses.map((item) => <option value={item} key={item}>{statusLabel(item)}</option>)}</select></div><div className="field"><label htmlFor="new-process-source">Origem</label><select className="form-select" id="new-process-source" value={source} onChange={(event) => setSource(event.target.value)}><option value="">Não informado</option>{candidateSources.map((item) => <option value={item} key={item}>{sourceLabel(item)}</option>)}</select></div></div>{selectedVacancy && <p className="panel-note">Empresa contratante: <strong>{selectedCompanyName ?? 'não definida'}</strong> · {selectedVacancy.quantity} {selectedVacancy.quantity === 1 ? 'posição cadastrada' : 'posições cadastradas'}.</p>}{error && <div className="form-error" role="alert" style={{ marginTop: 14 }}>{error}</div>}<div className="form-actions"><button type="button" className="button button-secondary" onClick={onCancel}>Cancelar</button><button className="button button-primary" disabled={loading}>{loading ? 'Criando…' : 'Criar processo'}<Icon name="git-branch" size={16} /></button></div></form>;
}

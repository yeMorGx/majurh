...
export async function POST(request: NextRequest) {
  try {
    ...
    if (password.length < 8 || password.length > 128) {
      return errorJson('A senha deve ter entre 8 e 128 caracteres.', 400);
    }
    ...
    try {
      ...
      await context.db`
        ...
        values (${context.organizationId}::uuid, ${authUserId}, ${email}, ${role}::public.app_role)
        on conflict (organization_id, user_id) do update
          set email = excluded.email, role = excluded.role
      `;
    } catch (error) {
      // Evita deixar um usuário órfão no Neon Auth caso o banco da aplicação
      // rejeite o perfil ou o vínculo com a organização.
      try {
        await auth.admin.removeUser({ userId: authUserId });
      } catch {
        // O erro original do banco é mais útil para o diagnóstico do deploy.
      }
      throw error;
    }
    ...
    return json({
      data: {
        user: { id: authUserId, name, email, role },
        organization: context.organization,
      },
    }, 201);
  } catch (error) {
    return databaseErrorResponse(error, { duplicateMessage: 'Este e-mail já possui um acesso.' });
  }
}
...
function authCreateErrorMessage(error: unknown) {
  ...
  if (code.includes('already') || message.includes('already') || message.includes('exists')) {
    return 'Este e-mail já possui um acesso no Neon Auth.';
  }
  if (code.includes('admin') || message.includes('forbidden') || message.includes('not allowed')) {
    return 'Sua conta não tem permissão de administrador no Neon Auth.';
  }

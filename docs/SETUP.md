...
A composição padrão da tela usa os assets `public/brand/majurh-dog-mark.svg` e `public/brand/majurh-login-art.png`. Os campos permanecem inputs reais com labels e placeholders orientativos; o controle circular da senha alterna sua visibilidade. Os quatro quadrados inferiores são controles sem ação, reservados para futuras formas de login. A organização pode substituir logo e banner somente por upload de arquivo, sem URL de imagem editável.

...
O logo principal da tela usa Three.js em um canvas transparente isolado. No hover, foco ou toque, a marca faz uma rotação 3D curta no eixo Y e retorna centralizada; os métodos alternativos permanecem estáticos. O componente usa o SVG local como textura, mantém um fallback enquanto carrega e respeita `prefers-reduced-motion`.
